...
**고객**
- 홈 화면 히어로 이미지 슬라이드, 공지 바, 이벤트 노출
- 메뉴 검색(자동완성 포함)/카테고리 필터/정렬/가격대 필터, 즐겨찾기, 최근 본 메뉴, 인기 메뉴 뱃지
- 메뉴 클릭 시 우측 슬라이드 패널에서 옵션 선택 후 담기, "바로 주문하기"로 담긴 메뉴 전부 결제 페이지로 이동
- 장바구니 담기(같은 메뉴도 옵션 다르게 여러 번 담기) → 배달/매장수령 선택 → 주문 → 상태 확인 → 재주문
- 매장 수령 주문은 픽업 바코드 발급, 픽업 준비 완료 시 홈 화면에 알림 배너 노출
