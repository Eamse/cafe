...
> - **손대지 않은 것**: `js/cartPanel.js`/`css/cart-panel.css`(장바구니 슬라이드 패널)는 여러 페이지가 공유하는 컴포넌트라 그대로 둠 — 홈에서 카드 클릭해 패널을 열면 아직 기존(팀원) 톤 그대로 나옴. 통일하려면 이 공유 파일도 손봐야 해서 별도로 확인 필요.
