...
- [x] **메뉴 옵션(아이스/핫 등) 다른 조합으로 같은 메뉴를 연달아 담지 못하던 버그 수정**: 담기 패널에서 한 번 담으면 "담았습니다" 버튼이 영구적으로 잠겨서, 아이스 1개 담은 뒤 핫으로 바꿔서 또 담으려 해도 클릭이 씹히던 문제. 담은 뒤 버튼을 잠깐(0.9초)만 잠그고 자동 복구하도록 수정, 그 전에 옵션을 바꾸면 기다리지 않고 바로 다시 누를 수 있게 함. 실제로 아이스 1개+핫 1개가 장바구니에 별도 줄로 잘 담기는 것 확인
- [x] **홈 화면 "최근 본 메뉴" 전체삭제가 새로고침 전까지 화면에 남아있던 버그 수정**: 두 가지 원인이 겹쳐 있었음 — ① `.viewed-sidebar`(및 이번에 함께 점검한 `.notice-bar`/`.bulk-action-bar`/`.my-tab-panel`/`.profile-name-form`/`.recent-searches`/`.pickup-estimate`)에 걸린 `display: flex` 스타일이 `hidden` 속성보다 우선 적용되어 JS로 숨겨도 실제로는 안 숨겨지던 문제(전에 고친 픽업 알림 배너와 동일한 종류의 버그, 전 프로젝트에서 같은 패턴을 찾아 전부 `[hidden] { display:none }` 규칙 추가), ② 목록이 비었을 때 `section.hidden = true`만 하고 기존 DOM 내용은 안 지워서 예전 목록이 그대로 남아있던 문제(비었을 때 내용도 함께 비우도록 수정)
