import { getEvents, saveEvents } from "../../js/data.js";
...
function render() {
  ...
  listEl.innerHTML = events
    ...
    .map(
      (event) => `
    <div class="event-row-admin ${event.isEnded ? "is-ended" : ""}" data-id="${event.id}">
      <div class="event-thumb-admin" style="${event.image ? `background-image: url('${event.image}')` : ""}">
        ${event.isEnded ? `<span class="ended-watermark">종료된 이벤트입니다</span>` : ""}
      </div>
      <div class="event-info-admin">
        <div class="event-title-admin">
          ${escapeHtml(event.title)}
          ${event.isEnded ? `<span class="ended-tag">종료된 이벤트입니다</span>` : ""}
        </div>
